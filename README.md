...
Tests 61–64 make the applicable authorization policy itself latent. The cheapest unresolved probe remains `q`, cost 1, and its residual branch contains both incompatible decisions and multiple policy hypotheses. This is useful as a boundary test, but runtime access-policy discovery is established in trust negotiation and an epistemic/contingent planner can augment its latent state with a policy identifier. Therefore unknown-policy state alone is not claimed as novel.
...
Controlled theorem datasets include `large_authorization_benchmark.csv`, `dynamic_proof_coupling.csv`, `evidence_authority_closure.csv`, `joint_coupling_matched_marginals.csv`, `joint_planning_collision.csv`, `authority_premium.csv`, `legitimate_pair_cut.csv`, `dynamic_authority_cut.csv`, `certificate_planner_comparison.csv`, `ocat_collision.csv`, and `policy_uncertainty.csv`, scaling through 10,000 worlds. `real_authorization_source_audit.csv` records real-source coverage. AuthBench remains real external-validity evidence, but it does not natively provide the counterfactual world×probe outcomes plus legitimate-executability/proof/policy-hypothesis labels required by these theorems; we do not fabricate them.
...
61. **Second-order cheapest unresolved probe** — q, cost 1; 9,900 residual incompatible pairs while two policy hypotheses remain.
62. **Policy-as-latent-state regression** — explicit `(world, policy)` state retained.
63. **Joint decision/policy uncertainty after q** — residual branch retains incompatible decisions and policy uncertainty.
64. **10,000-world second-order stress** — q cost 1; 24,995,000 residual incompatible pairs; two policy hypotheses remain.

## Current novelty status
**Do not write the paper yet.** Second-order policy uncertainty by itself collides with runtime policy discovery and state augmentation in epistemic/contingent planning. The next stop-and-write candidate must be stronger: self-referential evidence legitimacy, where determining which policy governs an evidence action requires evidence whose own legitimacy depends on that unresolved policy. This must survive trust negotiation, recursive authorization/fixed-point semantics, dynamic epistemic logic, and full-state planning before it can be called a breakthrough.
